...
def GenIdx(train_rgb_label, train_depth_label):
    rgb_pos = []
    unique_label_rgb = np.unique(train_rgb_label)

    for i in range(len(unique_label_rgb)):
        tmp_pos = [k for k,v in enumerate(train_rgb_label) if v==unique_label_rgb[i]]
        rgb_pos.append(tmp_pos)
        
    depth_pos = []
    unique_label_depth = np.unique(train_depth_label)

    for i in range(len(unique_label_depth)):
        tmp_pos = [k for k,v in enumerate(train_depth_label) if v==unique_label_depth[i]]
...
class IdentitySampler(Sampler):
    ...
    """
    def __init__(self, train_rgb_label, train_depth_label, rgb_pos, depth_pos, num_pos, batchSize, epoch):
