...
from make_single_stream_resnet_model import single_stream_net
...
network = single_stream_net(n_class, arch=args.arch)
network.to(device)
...
if args.optim == 'sgd':
    ...
    optimizer = optim.SGD([
        # Stem block layers 
        {'params': network.stem_block_resnet.parameters(), 'lr': 0.1 * cfg.BASE_LR},
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        fused_features= torch.cat((input1, input2), 0)
        
        features, scores = network(fused_features)

        loss_id = criterion_id(scores, labels.long())
        loss_tri, batch_acc = criterion_tri(features, labels)
        ...
        if (batch_idx + 1) % 32 == 0 :
            print('Epoch: [{}][{}/{}] '
                  ...
                  'Accu: {:.2f}'.format(
                epoch, (batch_idx+1), len(trainloader), optimizer.state_dict()['param_groups'][3]['lr'],
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            pool_feat, bn_feat = network(input)
